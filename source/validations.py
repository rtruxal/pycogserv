...
class QueryChecker():
    ...
    def check_web_params(query_dict, header_dict):
        ...
        if 'count' in query_dict.keys() and query_dict['count']:
        ...
        if 'safeSearch' in query_dict.keys() and query_dict['safeSearch']:
            ...
            if 'X-Search-ClientIP' in header_dict.keys() and header_dict['X-Search-ClientIP']:
        ...
        if 'setLang' in query_dict.keys():
            if 'Accept-Language' in header_dict.keys() and header_dict['Accept-Language']:
                raise AssertionError('Attempt to use both language header and query param.')
        if 'textDecorations' in query_dict.keys() and query_dict['textDecorations']:
            if query_dict['textDecorations'].lower() not in ('true', 'false'):
                raise TypeError('textDecorations is type bool')
        if 'textFormat' in query_dict.keys() and query_dict['textFormat']:
